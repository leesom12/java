...
<%
	...
	ArrayList<NewsDto> arr= dao.newsList(select, search, start, end);
%>
	
<script type="text/javascript">
	function viewNews(no){
		news.t_no.value=no;
		news.method="post";
	  ...
	  <div class="bord_list">
		<table class="bord_table" summary="이표는 번호,제목,글쓴이,날자,조회수로 구성되어 있습니다">
			...
			<tbody>
				...
				%>
					<tr>
						<td><a href="javascript:viewNews('<%=arr.get(k).getNo() %>')"><%=totalCount-((current_page-1)*list_setup_count+k) %></a></td>
						<td class="title"><a href="javascript:viewNews('<%=arr.get(k).getNo() %>')"><%=arr.get(k).getTitle() %></a></td>
